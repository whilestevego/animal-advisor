...
class MemeDefinition {
  ...
  static find (sentence) {
    ...
    const memeDefinition = _.find(
      definitionList,
      memeDef => memeDef.pattern.test(sentence)
    )

    if (_.isUndefined(memeDefinition)) {
      throw new Error('Meme could not be found')
    }

    return new MemeDefinition(_.merge({sentence}, memeDefinition))
  }
...
const definitionList = [
  // {
  //   // Match custom 'x|y|z' meme first or it may matched by another pattern.
