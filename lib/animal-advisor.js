...
function create (cachePath) {
  function say (sentence) {
    sentence = sentence ? sentence.trim() : null
    return generateAdviceAnimal(...MemeDefinition.find(sentence).toMemeCaptainParams())
  }

  function generateAdviceAnimal (sourceImage, topText, bottomText) {
    console.log(`Generating ${sourceImage}`)
    return Promise.resolve(Captain.createMeme(sourceImage, topText, bottomText)).then(delay(200)).then(downloadImage)
  }
